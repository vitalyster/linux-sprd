...
/**** ZTE_ADD FOR CPUFREEZER begin ****/
#include <linux/list.h>
/**** ZTE_ADD FOR CPUFREEZER end ****/
...
{
	struct freeze_uid *temp = NULL;
	struct freeze_uid *node = NULL;

	if (uid_list == NULL) {
		pr_err("could not delete freezer list for uidlist is NULL!!!\n");
		return;
	}
	list_for_each_entry_safe(temp, node, &(uid_list->freezer_list), freezer_list)
...
static void freezer_change_state(struct freezer *freezer, bool freeze)
{
	...
	rcu_read_unlock();
/**** ZTE_ADD FOR CPUFREEZER begin ****/
	if (cgroup_get_uid(freezer) > FIRST_APPLICATION_UID) {
		if (uid_list == NULL)
