...
#include <vendor/common/smartpa_stat.h>
#include "aw881xx.h"
...
#define AW881XX_VERSION "v0.1.14"
...
#define AW_DELAY_1_MS	1	/* 1ms */
#define AW_DELAY_2_MS	2	/* 2ms */
#define AW_DELAY_10_MS	10	/* 10ms */
...
static uint16_t aw881xx_base_addr[] = {
	...
	AW881XX_DSPBYPASS_REG_ADDR,
	AW881XX_DSPBYPASS_DSP_FW_ADDR,
	AW881XX_DSPBYPASS_DSP_CFG_ADDR
};
...
static char aw881xx_cfg_name[][AW881XX_CFG_NAME_MAX] = {
	...
	/* dsp_bypass */
	{"aw881xx_pid_xx_dspbypass_reg.bin"},
	{"aw881xx_pid_01_dspbypass_fw.bin"},
	{"aw881xx_pid_01_dspbypass_cfg.bin"},
};

/******************************************************
 *
 * add for sprd platform stereo sound
 *
 ******************************************************/
#define AW881XX_SUPPORT_CODEC_MAX		(4)
struct aw881xx *g_codec_aw881xx[AW881XX_SUPPORT_CODEC_MAX] = {
	NULL, NULL, NULL, NULL};
...
{
	int cfg_idx = aw881xx->cfg_num;
	struct aw881xx_container *aw881xx_cfg = &aw881xx->cfg_info[cfg_idx + AW881XX_DSP_CFG_FILE_OFFSET].cfg_data;
...
/* add for tx cfg */
static void aw881xx_set_iis_tx(struct aw881xx *aw881xx, bool enable)
{
	aw881xx_reg_write_bits(aw881xx, AW881XX_REG_I2SCFG1, AW881XX_BIT_I2SCFG1_TXEN_MASK,
			(enable ? AW881XX_BIT_I2SCFG1_TXEN_ENABLE : AW881XX_BIT_I2SCFG1_TXEN_DISABLE));
}

static int  aw881xx_request_cfg_file(struct aw881xx *aw881xx, int cfg_idx);
...
static int aw881xx_start(struct aw881xx *aw881xx)
{
	...
	if (ret < 0) {
		...
		aw881xx->is_power_on = AW881XX_PA_CLOSE_ST;
	} else {
		...
		aw881xx_set_iis_tx(aw881xx, true);
		...
		aw881xx->is_power_on = AW881XX_PA_OPEN_ST;
	}
...
static void aw881xx_stop(struct aw881xx *aw881xx)
{
	...
	aw881xx->is_power_on = AW881XX_PA_CLOSING_ST;
	ret = aw881xx_get_sysint(aw881xx, &sysint);
	if (ret < 0)
	...
	aw881xx_set_iis_tx(aw881xx, false);
...
/* check chip state */
static void aw88xx_dump_chip_state(struct aw881xx *aw881xx)
{
	unsigned int i = 0;
	uint16_t reg_val = 0;
	...
	aw_dev_info(aw881xx->dev, "%s reg[0x%02x] = 0x%02x\n",
		__func__, AW881XX_REG_SYSINT, reg_val);

	mutex_lock(&aw881xx->i2c_lock);
	/* dump reg */
	pr_info("aw881xx reg dump start --------------\n");
	...
	*/
	...
	/*for (retry_cnt = 0; retry_cnt < 5; retry_cnt++)*/ {
		...
		for (i = 0; i < aw881xx_cont->len;
			...
			aw881xx_i2c_reads(aw881xx, AW881XX_REG_DSPMDAT,
				dsp_data, tmp_len);
...
{
	int ret = -1;
	int cfg_idx = aw881xx->cfg_num + AW881XX_DSP_CFG_FILE_OFFSET;
	struct aw881xx_container *aw881xx_cfg = NULL;

	aw_dev_info(aw881xx->dev, "%s enter cfg_name=%s\n", __func__, aw881xx->cfg_name[cfg_idx]);

	mutex_lock(&aw881xx->lock);
	if (aw881xx->cfg_info[cfg_idx].cfg_load_st != AW881XX_CFG_READY) {
		/* request cfg */
		ret = aw881xx_request_cfg_file(aw881xx, cfg_idx);
		if (ret < 0) {
			aw_dev_info(aw881xx->dev, "%s: reg cfg not readey\n", __func__);
			mutex_unlock(&aw881xx->lock);
			return FAIL;
		}
	}

	aw881xx_cfg = &aw881xx->cfg_info[cfg_idx].cfg_data;
	if (aw881xx->work_flag == false && aw881xx_cfg->data) {
		aw_dev_info(aw881xx->dev, "%s: mode_flag = %d %s\n",
				__func__, aw881xx->work_flag, aw881xx_cfg->data?"":"cfg is NULL");
		mutex_unlock(&aw881xx->lock);
		return FAIL;
	}
	aw881xx_dsp_container_update(aw881xx, aw881xx_cfg,
				aw881xx_base_addr[cfg_idx]);
...
{
	int ret = -1;
	int cfg_idx = aw881xx->cfg_num + AW881XX_DSP_FW_FILE_OFFSET;
	struct aw881xx_container *aw881xx_cfg = NULL;
	...
	aw881xx_cfg = &aw881xx->cfg_info[cfg_idx].cfg_data;
	if (aw881xx->work_flag == true && aw881xx_cfg->data) {
		mutex_lock(&aw881xx->lock);
		aw881xx_run_mute(aw881xx, true);
		aw881xx_dsp_enable(aw881xx, false);
		aw881xx_memclk_select(aw881xx, AW881XX_MEMCLK_PLL);
		/* update dsp fw to chip */
		if (!aw881xx->is_dsp_fw_updated) {
			aw881xx_dsp_container_update(aw881xx, aw881xx_cfg,
					aw881xx_base_addr[cfg_idx]);
			aw881xx->is_dsp_fw_updated = true;
		} else {
			aw_dev_info(aw881xx->dev, "%s dsp fw is updated, not updade again\n", __func__);
		}
		mutex_unlock(&aw881xx->lock);
...
{
	int cfg_idx = aw881xx->cfg_num + AW881XX_REG_FILE_OFFSET;
	int ret = 0;
	struct aw881xx_container *cont = NULL;
	...
	cont = &aw881xx->cfg_info[cfg_idx].cfg_data;
	if (aw881xx->work_flag == true &&
		aw881xx->cfg_info[cfg_idx].cfg_load_st == AW881XX_CFG_READY) {
		aw881xx_reg_container_update(aw881xx, cont);
		aw_dev_info(aw881xx->dev, "%s: load reg success\n", __func__);
	}
	mutex_unlock(&aw881xx->lock);
	return 0;
}
...
static int aw881xx_i2s_check(struct aw881xx *aw881xx)
{
	int i = 0;
	int ret = -1;
	int pass_cnt = 0;

	for (i = 0; i < AW881XX_IIS_CHECK_MAX_CNT; i++) {
		mutex_lock(&aw881xx->lock);
		ret = aw881xx_get_iis_status(aw881xx);
		mutex_unlock(&aw881xx->lock);
		if (ret < 0) {
			...
			pass_cnt++;
			if (pass_cnt > AW881XX_IIS_CHECK_PASS_CNT) {
				aw_dev_info(aw881xx->dev,
						"%s: i2s check pass cnt = %d\n", __func__, i);
				return 0;
			}
		}
	}
	aw_dev_err(aw881xx->dev,
				"%s: i2s check failed, ret=%d\n",
				__func__, ret);
	return ret;
}

static int aw881xx_update_all_cfg(struct aw881xx *aw881xx)
{
	int ret = -1;
	aw_dev_info(aw881xx->dev, "%s: enter\n", __func__);

	aw881xx_load_reg(aw881xx);

	ret = aw881xx_i2s_check(aw881xx);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev,
				"%s: iis check failed, entry muteret=%d\n",
				__func__, ret);

		aw881xx_run_mute(aw881xx, true);
	} else {
		aw_dev_info(aw881xx->dev, "%s: update dsp and cfg\n", __func__);
		aw881xx_load_dsp_fw(aw881xx);
		ret = aw881xx_load_dsp_cfg(aw881xx);
	}
	if (ret < 0) {
		aw88xx_dump_chip_state(aw881xx);
	}
...
static void aw881xx_cold_start(struct aw881xx *aw881xx)
{
	...
	ret = aw881xx_update_all_cfg(aw881xx);
	if (ret < 0) {
		aw_dev_err(aw881xx->dev,
				"%s:update_all_cfg failed: %d\n",
				__func__, ret);
	}
}
...
static void aw881xx_delay_work_routine(struct work_struct *work)
{
	...
	aw_dev_info(aw881xx->dev, "%s enter, work_flag = %d\n", __func__, aw881xx->work_flag);
	...
	aw_dev_info(aw881xx->dev, "%s aw881xx_delay_work_routine exit\n", __func__);
}
...
void aw881xx_smartpa_cfg(struct aw881xx *aw881xx, bool flag)
{
	...
	aw_dev_info(aw881xx->dev, "%s: audio_stream_st=%d, aw881xx_pa_switch = %d, aw881xx->is_power_on = %d",
			__func__, aw881xx->audio_stream_st,
			aw881xx->aw881xx_pa_switch, aw881xx->is_power_on);

	if (aw881xx->work_flag == true) {  /* enter work */
		if (aw881xx->aw881xx_pa_switch == AW881XX_ON_PA) {
			if (aw881xx->is_power_on == AW881XX_PA_CLOSE_ST) {
				/*aw881xx->is_power_on = AW881XX_PA_OPENING_ST;*/
				schedule_delayed_work(&aw881xx->delay_work,
					msecs_to_jiffies(AW881XX_START_WORK_DELAY_MS));
			}
		}
	} else { /* enter mute */
		/*cancel_delayed_work_sync(&aw881xx->delay_work);*/
		if (aw881xx->is_power_on == AW881XX_PA_OPEN_ST) {
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	int i = 0;
	struct aw881xx **aw881xx_codecs = aw881xx->codecs;
	if (aw881xx->codecs_num > 1) {
		for (i = 0; i < aw881xx->codecs_num; i++) {
			if (strnstr(kcontrol->id.name, aw881xx_codecs[i]->name_suffix, SNDRV_CTL_ELEM_ID_NAME_MAXLEN)) {
				aw881xx = aw881xx_codecs[i];
				break;
			}
		}
	}
...
static const char *const pa_switch_function[] = {
	"Off",
	"On",
};
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	aw_dev_info(aw881xx->dev, "%s: scene_mode=%d\n",
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == aw881xx->scene_mode) {
		aw_dev_info(aw881xx->dev, "%s:hold scene mode = %d\n",
				__func__, aw881xx->scene_mode);
		return 1;
	}
	...
	aw881xx_get_cfg_shift(aw881xx);
	aw_dev_info(aw881xx->dev, "%s:switch new scene mode = %d\n",
			__func__, aw881xx->scene_mode);
...
static int aw881xx_pa_switch_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int i = 0;
	aw_snd_soc_codec_t *codec =
	...
	pa_switch_ptr = &aw881xx->aw881xx_pa_switch;

	aw_dev_info(aw881xx->dev, "%s: audio_stream_st=%d, aw881xx_pa_switch = %d, is_power_on = %d",
			__func__, aw881xx->audio_stream_st,
			aw881xx->aw881xx_pa_switch, aw881xx->is_power_on);
...
static int aw881xx_pa_switch_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	int i = 0;
	aw_snd_soc_codec_t *codec =
	...
	aw881xx_pa_switch = &aw881xx->aw881xx_pa_switch;
	aw_dev_info(aw881xx->dev, "%s: ucontrol->value.integer.value[0]=%ld\n",
			__func__, ucontrol->value.integer.value[0]);

	if (ucontrol->value.integer.value[0] == *aw881xx_pa_switch) {
		aw_dev_info(aw881xx->dev, "%s: aw881xx_pa_switch not change, return\n", __func__);
		return 1;
	}

	*aw881xx_pa_switch = ucontrol->value.integer.value[0];

	aw_dev_info(aw881xx->dev, "%s: audio_stream_st=%d, aw881xx_pa_switch = %d, is_power_on = %d",
			__func__, aw881xx->audio_stream_st,
			*aw881xx_pa_switch, aw881xx->is_power_on);
	cancel_delayed_work_sync(&aw881xx->delay_work);
	mutex_lock(&aw881xx->lock);
	if ((*aw881xx_pa_switch == AW881XX_OFF_PA) &&
		(aw881xx->is_power_on == AW881XX_PA_OPEN_ST)) {
		aw881xx_stop(aw881xx);
	} else if ((*aw881xx_pa_switch == AW881XX_ON_PA) &&
		(aw881xx->is_power_on == AW881XX_PA_CLOSE_ST)) {
		/*aw881xx->is_power_on = AW881XX_PA_OPENING_ST;*/
		/*aw881xx_start(aw881xx);*/
		if (aw881xx->audio_stream_st == AW881XX_AUDIO_START) {
			aw881xx_smartpa_cfg(aw881xx, true);
		}
	} else {
		aw_dev_info(aw881xx->dev, "%s: do noting\n", __func__);
	}
	mutex_unlock(&aw881xx->lock);
	return 0;
}
...
static struct snd_kcontrol_new aw881xx_controls[] = {
	...
	SOC_ENUM_EXT("aw881xx_pa_switch", aw881xx_snd_enum[1],
			aw881xx_pa_switch_get, aw881xx_pa_switch_set),
};
				...
				struct snd_kcontrol_new *src_control, int num)
{
	...
	dst_control = devm_kzalloc(aw881xx->dev,
					...
					GFP_KERNEL);
	if (!dst_control) {
		aw_dev_err(aw881xx->dev, "%s:kcontrol kzalloc failed\n",
			__func__);
		return;
	}
	...
	ret = aw_componet_codec_ops.aw_snd_soc_add_codec_controls(aw881xx->codec,
							dst_control, num);
	if (ret) {
		aw_dev_err(aw881xx->dev, "%s:add kcontrol failed, name = %s\n",
			__func__, dst_control[i].name);
	}
}
			...
			struct snd_soc_dai *dai)
{
	...
	aw881xx->startup_cnt++;
	aw_dev_info(aw881xx->dev, "%s: enter, stream = %d, startup_cnt = %d\n",
			__func__, substream->stream, aw881xx->startup_cnt);

	for (i = 0; i < aw881xx->codecs_num; i++) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			mutex_lock(&aw881xx_codecs[i]->lock);
			aw881xx_run_pwd(aw881xx_codecs[i], false);
			mutex_unlock(&aw881xx_codecs[i]->lock);
		}
	}
	return 0;
}
				...
				struct snd_soc_dai *dai)
{
	...
	/* dump audio param */
	aw_dev_info(aw881xx->dev, "%s: rate = %d\n", __func__, params_rate(params));
	aw_dev_info(aw881xx->dev, "%s: width = %d\n", __func__, params_width(params));
	aw_dev_info(aw881xx->dev, "%s: physical width = %d\n", __func__, params_physical_width(params));
	aw_dev_info(aw881xx->dev, "%s: channel nums = %d\n", __func__, params_channels(params));

	for (i = 0; i < aw881xx->codecs_num; i++) {
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
			aw_dev_dbg(aw881xx_codecs[i]->dev,
		...
		/* get rate param */
		/*aw881xx_codecs[i]->rate = params_rate(params);*/
		aw881xx_codecs[i]->rate = 48000;
		aw_dev_dbg(aw881xx_codecs[i]->dev, "%s: requested rate: %d, sample size: %d\n",
				__func__, aw881xx_codecs[i]->rate,
				snd_pcm_format_width(params_format(params)));

		/* get bit width */
		/*aw881xx_codecs[i]->width = params_width(params);*/
		aw881xx_codecs[i]->width = 32;
		aw_dev_dbg(aw881xx_codecs[i]->dev, "%s: width = %d\n", __func__, aw881xx_codecs[i]->width);

		mutex_lock(&aw881xx_codecs[i]->lock);
		aw881xx_update_hw_params(aw881xx_codecs[i]);
		mutex_unlock(&aw881xx_codecs[i]->lock);
	}
...
static int aw881xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	for (i = 0; i < aw881xx->codecs_num; i++) {
		if (!(aw881xx_codecs[i]->flags & AW881XX_FLAG_START_ON_MUTE))
			return 0;
		cancel_delayed_work_sync(&aw881xx_codecs[i]->delay_work);
		if (mute) {
			if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
				mutex_lock(&aw881xx_codecs[i]->lock);
				aw881xx_smartpa_cfg(aw881xx_codecs[i], false);
				mutex_unlock(&aw881xx_codecs[i]->lock);
				aw881xx_codecs[i]->audio_stream_st = AW881XX_AUDIO_STOP;
			}
		} else {
			if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
				mutex_lock(&aw881xx_codecs[i]->lock);
				aw881xx_smartpa_cfg(aw881xx_codecs[i], true);
				mutex_unlock(&aw881xx_codecs[i]->lock);
				aw881xx_codecs[i]->audio_stream_st = AW881XX_AUDIO_START;
			}
		}
	}
				...
				struct snd_soc_dai *dai)
{
	...
	if (aw881xx->startup_cnt > 0)
		aw881xx->startup_cnt--;
	aw_dev_info(aw881xx->dev, "%s stream %d, startup_cnt %d\n", __func__, substream->stream, aw881xx->startup_cnt);
	...
	for (i = 0; i < aw881xx->codecs_num; i++) {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			aw881xx_codecs[i]->rate = 0;
			mutex_lock(&aw881xx_codecs[i]->lock);
			aw881xx_run_pwd(aw881xx_codecs[i], true);
			mutex_unlock(&aw881xx_codecs[i]->lock);
		}
	}
}
...
static int aw881xx_probe(aw_snd_soc_codec_t *codec)
{
	...
	aw_dev_info(aw881xx->dev, "%s: enter\n", __func__);
	for (i = 0; i < aw881xx->codecs_num; i++) {
		aw881xx_codecs[i]->codec = codec;
		aw881xx_add_codec_controls(aw881xx_codecs[i]);
	}
 ...
 *
 ******************************************************/
static void aw881xx_update_cfg_data_order(struct aw881xx_container *aw881xx_cfg)
{
	int i = 0;
	uint8_t tmp_val = 0;
...
static int  aw881xx_request_cfg_file(struct aw881xx *aw881xx, int cfg_idx)
{
	...
	cfg_file_name = aw881xx->cfg_name[cfg_idx];
	if (cfg_file_name) {
		aw_dev_info(aw881xx->dev, "%s start load %s\n", __func__, cfg_file_name);
		for (retry_cnt = 0; retry_cnt < AW881XX_REQUEST_CFG_MAX_CNT; retry_cnt++) {
			ret = request_firmware(&fw, cfg_file_name, aw881xx->dev);
			if (ret < 0) {
				if (fw != NULL) {
					release_firmware(fw);
				}
				ret = aw_dev_err(aw881xx->dev, "%s try preload %s failed, try_cnt = %d, ret = %d",
					__func__, cfg_file_name, retry_cnt, ret);
				msleep(AW_DELAY_10_MS);
			} else {
				break;
			}
		}
		/* request firmware failed */
		if (retry_cnt == AW881XX_REQUEST_CFG_MAX_CNT) {
			ret = aw_dev_err(aw881xx->dev, "%s preload %s failed",
				__func__, cfg_file_name);
			ret = -ENOENT;
			goto err_request;
		}
		aw881xx->cfg_info[cfg_idx].cfg_data.data = devm_kzalloc(aw881xx->dev,
				fw->size + sizeof(int), GFP_KERNEL);
		if (!aw881xx->cfg_info[cfg_idx].cfg_data.data) {
			aw_dev_err(aw881xx->dev, "%s: error allocating memory\n",
				__func__);
			ret = -ENOMEM;
			goto err_mem_alloc;
		}
		aw881xx->cfg_info[cfg_idx].cfg_data.len = fw->size;
		memcpy(aw881xx->cfg_info[cfg_idx].cfg_data.data, fw->data, fw->size);
		release_firmware(fw);
		aw881xx->cfg_info[cfg_idx].cfg_load_st = AW881XX_CFG_READY;
		aw_dev_info(aw881xx->dev, "%s preload file %s success, len = %d\n",
				__func__, cfg_file_name, aw881xx->cfg_info[cfg_idx].cfg_data.len);

		if ((cfg_idx % AW881XX_MODE_CFG_NUM_MAX) == AW881XX_DSP_FW_FILE_OFFSET ||
			((cfg_idx % AW881XX_MODE_CFG_NUM_MAX) == AW881XX_DSP_CFG_FILE_OFFSET)) {
			aw_dev_info(aw881xx->dev, "%s update %s data order\n",
					__func__, cfg_file_name);
			aw881xx_update_cfg_data_order(&aw881xx->cfg_info[cfg_idx].cfg_data);
		}
	}
	return 0;
err_mem_alloc:
	release_firmware(fw);
err_request:
	aw881xx->cfg_info[cfg_idx].cfg_load_st = AW881XX_CFG_NOT_LOAD;
	aw881xx->cfg_info[cfg_idx].cfg_data.data = NULL;
	return ret;
}


static void aw881xx_delay_work_load_cfg(struct work_struct *work)
{
	int ret = -1;
	struct aw881xx *aw881xx =
		container_of(work, struct aw881xx, load_cfg_work.work);
	int i = 0;
	aw_dev_info(aw881xx->dev, "%s enter\n", __func__);
	mutex_lock(&aw881xx->lock);
	aw881xx_read_dsp_pid(aw881xx);
	for (i = 0; i < AW881XX_CFG_NUM_MAX; i++) {
		ret = aw881xx_request_cfg_file(aw881xx, i);
		if (ret < 0) {
			aw_dev_err(aw881xx->dev, "%s preload file %s filed",
					__func__, aw881xx->cfg_name[i]);
			continue;
		}
	}
	mutex_unlock(&aw881xx->lock);
					...
					struct device_node *np)
{
	...
	if (!strcmp(channel_value, "left")) {
		aw881xx->name_suffix = "left";
	} else if (!strcmp(channel_value, "right")) {
		aw881xx->name_suffix = "right";
	} else {
		aw_dev_info(aw881xx->dev,
...
static int aw881xx_parse_codec_info_dt(struct aw881xx *aw881xx,
					struct device_node *np)
{
	int ret;

	ret = of_property_read_u32(np, "codec-id", &aw881xx->codec_id);
	if (ret) {
		dev_err(aw881xx->dev,
			"%s: find no codec-id\n",
			__func__);
		...
		dev_info(aw881xx->dev, "%s: codec-id = %d\n",
			__func__, aw881xx->codec_id);
	}

	ret = of_property_read_u32(np, "codecs-num", &aw881xx->codecs_num);
	if (ret) {
		dev_err(aw881xx->dev,
			"%s: find no codecs-num, use default codecs_num\n",
			__func__);
		aw881xx->codecs_num = AW881XX_SUPPORT_CODEC_MAX;
	} else {
		dev_info(aw881xx->dev, "%s: codecs-num = %d\n",
			__func__, aw881xx->codecs_num);
	}
				...
				struct device_node *np)
{
	...
	ret = aw881xx_parse_codec_info_dt(aw881xx, np);
	return ret;
}
					...
					const char *buf, size_t count)
{
	...
	if (val) {
		aw881xx->init = AW881XX_INIT_ST;
		cfg_idx = aw881xx->cfg_num + AW881XX_DSP_FW_FILE_OFFSET;
		aw881xx->cfg_info[cfg_idx].cfg_load_st = AW881XX_CFG_NOT_LOAD;
		cfg_idx = aw881xx->cfg_num + AW881XX_DSP_CFG_FILE_OFFSET;
		aw881xx->cfg_info[cfg_idx].cfg_load_st = AW881XX_CFG_NOT_LOAD;
		aw881xx->is_dsp_fw_updated = false;
		cancel_delayed_work_sync(&aw881xx->delay_work);
				...
				struct device_attribute *attr, char *buf)
{
	...
	struct aw881xx_container *aw881xx_fw =
			&aw881xx->cfg_info[cfg_idx + AW881XX_DSP_FW_FILE_OFFSET].cfg_data;
	struct aw881xx_container *aw881xx_fw_cfg =
			&aw881xx->cfg_info[cfg_idx + AW881XX_DSP_CFG_FILE_OFFSET].cfg_data;
	...
	if (aw881xx->dsp_cfg == AW881XX_DSP_BYPASS) {
		...
		if (ret < 0) {
			...
			aw881xx_i2c_write(aw881xx, AW881XX_REG_DSPMADD,
					AW881XX_SPK_DSP_CFG_ADDR);
			for (i = 0; i < aw881xx_fw_cfg->len; i += 2) {
				aw881xx_i2c_read(aw881xx,
						AW881XX_REG_DSPMDAT, &reg_val);
				len += snprintf(buf + len, PAGE_SIZE - len,
			     ...
			     const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&aw881xx->load_cfg_work, aw881xx_delay_work_load_cfg);
	if (aw881xx->codec_id < AW881XX_SUPPORT_CODEC_MAX) {
		g_codec_aw881xx[aw881xx->codec_id] = aw881xx;
	}
	aw881xx->codecs = g_codec_aw881xx;
	aw881xx->aw881xx_pa_switch = AW881XX_OFF_PA; /*can't open*/
	aw881xx->is_power_on = AW881XX_PA_CLOSE_ST;
	aw881xx->audio_stream_st = AW881XX_AUDIO_STOP;
	aw881xx->startup_cnt = 0;
	aw881xx->scene_mode = AW881XX_OFF_MODE;
	aw881xx->is_dsp_fw_updated = false;
	schedule_delayed_work(&aw881xx->load_cfg_work,
