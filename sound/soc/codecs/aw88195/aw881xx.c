...
static void aw881xx_dsp_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	if (aw881xx->dsp_cfg_cnt != NULL) {
		devm_kfree(aw881xx->dev, aw881xx->dsp_cfg_cnt);
	}
	aw881xx->dsp_cfg_cnt = NULL;
	aw881xx->dsp_cfg_cnt = devm_kzalloc(aw881xx->dev, cont->size + sizeof(int), GFP_KERNEL);
	if (!aw881xx->dsp_cfg_cnt) {
		release_firmware(cont);
...
static void aw881xx_reg_loaded(const struct firmware *cont, void *context)
{
	...
	if (ret < 0) {
		aw_dev_err(aw881xx->dev,
				"%s: max number of retries has been reached, dsp is NOT update\n",
				__func__);
	}
}
...
{
	int delay_time = AW881XX_START_WORK_DELAY_MS;
	aw_dev_info(aw881xx->dev, "%s: flag = %d\n", __func__, flag);

	if ((aw881xx->work_flag == false) && (flag == false)) {
		aw_dev_dbg(aw881xx->dev, "%s: already stopped\n", __func__);
		return;
	}

	aw881xx->work_flag = flag;
	if (flag == true && aw881xx->scene_mode != AW881XX_OFF_MODE) {
		if (aw881xx->scene_mode == AW881XX_VOICE_MODE)
			delay_time = AW881XX_START_WORK_DELAY_MS_VOICE;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == AW881XX_OFF_MODE) {
		mutex_lock(&aw881xx->lock);
