...
static void sprd_i2c_set_clk(struct sprd_i2c *i2c_dev, u32 freq)
{
	...
	u32 high = 0, low = 0, div0 = 0, div1 = 0;
	/*
	 ...
	 * low = (prescale * 2 * 3) / 5
	 */
/*ZTE modify 400K&100K duty 20200827*/
	if (freq == 400000) {
		high = ((i2c_dvd << 1) * 31) / 100;
		low = ((i2c_dvd << 1) * 69) / 100;
	} else if (freq == 100000) {
		high = ((i2c_dvd << 1) * 46) / 100;
		low = ((i2c_dvd << 1) * 54) / 100;
	} else {
		high = ((i2c_dvd << 1) * 2) / 5;
		low = ((i2c_dvd << 1) * 3) / 5;
	}
	div0 = I2C_ADDR_DVD0_CALC(high, low);
	div1 = I2C_ADDR_DVD1_CALC(high, low);
	...
	if (freq == 400000)
		writel((18 * apb_clk) / 10000000, i2c_dev->base + ADDR_STA0_DVD);
	else if (freq == 100000)
		writel((12 * apb_clk) / 1000000, i2c_dev->base + ADDR_STA0_DVD);
