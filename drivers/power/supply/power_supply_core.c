				  ...
				  struct power_supply_battery_info *info, int num)
{
	...
	dev_info(&psy->dev, "%s enter\n", __func__);
	dev_info(&psy->dev, "%s get battery%d info\n", __func__, num);
	...
	of_property_read_u32_index(battery_np, "charge-dcp-current-microamp", 1,
				   &info->cur.dcp_limit);
#ifdef ZTE_PROJECT_DCP_LIMIT_CONFIG
	info->cur.dcp_limit = ZTE_PROJECT_DCP_LIMIT_CONFIG;
#endif
	...
	dev_info(&psy->dev, "ocv-capacity-celsius, len %d, ocv_temp %d\n",
			len, info->ocv_temp[0]);
	...
	for (index = 0; index < len; index++) {
		...
		for (i = 0; i < tab_len; i++) {
			...
			dev_dbg(&psy->dev, "index %d: [%d]: ocv %d, cap: %d\n",
						index, i, table[i].ocv, table[i].capacity);
		}
	}

	list = of_get_property(battery_np, "voltage-temp-table", &size);
	if (!list || !size) {
		dev_info(&psy->dev, "%s no voltage-temp-table defined!\n", __func__);
		return 0;
	}
	...
	if (!list || !size) {
		dev_info(&psy->dev, "%s no capacity-temp-table defined!\n", __func__);
		return 0;
	}
	...
	if (!list || !size) {
		dev_info(&psy->dev, "%s no resistance-temp-table defined!\n", __func__);
		return 0;
	}
