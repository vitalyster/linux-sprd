...
static int sc27xx_fgu_get_boot_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	...
	ret = sc27xx_fgu_get_current(data, &cur_now);
	if (ret)
					      ...
					      int cap)
{
	...
	SPRD_FGU_DEBUG("%s: init_cap:%d, ocv:%d, batt_uV:%d, min_volt:%d, alarm_cap:%d\n",
			__func__, data->init_cap, ocv, batt_uV, data->min_volt, data->alarm_cap);
}
