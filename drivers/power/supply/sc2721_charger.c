...
#define SC2721_CHG_CURRENT_MAX			1300
...
#define SC2721_CHG_TERM_CURRENT_RATIO_10	3
#define SC2721_CHG_TERM_CURRENT_RATIO_20	2
#define SC2721_CHG_TERM_CURRENT_RATIO_40	1
#define SC2721_CHG_TERM_CURRENT_RATIO_90	0
...
static int sc2721_charger_get_current(struct sc2721_charger_info *info, u32 *cur);
					  ...
					  u32 vol)
{
	...
	if (vol >= SC2721_TERM_VOLTAGE_MIN) {
		...
		dev_info(info->dev, "#1 %s: vol=%dmv, big_level=%d, small_level=%d, cv=%d, calib_data=%d\n",
			__func__, vol, big_level, small_level, cv, calib_data);
		...
		dev_info(info->dev, "#2 %s: vol=%dmv, big_level=%d, small_level=%d, cv=%d, calib_data=%d\n",
			__func__, vol, big_level, small_level, cv, calib_data);
	...
	dev_info(info->dev, "#3 %s: vol=%dmv, big_level=%d, small_level=%d, cv=%d, calib_data=%d\n",
			__func__, vol, big_level, small_level, cv, calib_data);
...
#define CHG_TERM_CURRENT_RATIO_10 10
#define CHG_TERM_CURRENT_RATIO_20 20
#define CHG_TERM_CURRENT_RATIO_40 40
#define CHG_TERM_CURRENT_RATIO_90 90

#define CHG_TERM_CURRENT_MAX_UA 200000
#define CHG_TERM_CURRENT_MIN_UA 100000

static int sc2721_set_termination_current(struct sc2721_charger_info *info, u32 term_cur)
{
	int val = 0, ret = 0;
	int ratio = 0;

	ratio = term_cur * 100 / (info->cur.dcp_cur / 1000);

	if (ratio <= CHG_TERM_CURRENT_RATIO_10)
		val = SC2721_CHG_TERM_CURRENT_RATIO_10;
	else if (ratio <= CHG_TERM_CURRENT_RATIO_20)
		val = SC2721_CHG_TERM_CURRENT_RATIO_20;
	else if (ratio <= CHG_TERM_CURRENT_RATIO_40)
		val = SC2721_CHG_TERM_CURRENT_RATIO_40;
	else
		val = SC2721_CHG_TERM_CURRENT_RATIO_90;

	dev_info(info->dev, "%s: dcp curr: %d, aimed term curr: %d, set val: %d\n",
			  __func__, info->cur.dcp_cur, term_cur, val);

	/* Set charge termination current */
	ret = regmap_update_bits(info->regmap,
				info->base + SC2721_CHG_CFG0,
				SC2721_CHG_TERMINATION_CURRENT_MASK,
				val << SC2721_CHG_TERMINATION_CURRENT_MASK_SHIT);
	if (ret) {
...
static int sc2721_get_termination_current(struct sc2721_charger_info *info,
					u32 *term_cur)
{
	int ret = 0;
	u32 val = 0, term_cur_set = 0;
	int chg_cur = 0;
	int ratio = 0;
	...
	ret = sc2721_charger_get_current(info, &chg_cur);
	if (ret) {
		dev_err(info->dev, "failed to read charge current\n");
	}

	term_cur_set = (val & SC2721_CHG_TERMINATION_CURRENT_MASK) >> SC2721_CHG_TERMINATION_CURRENT_MASK_SHIT;
	if (term_cur_set == SC2721_CHG_TERM_CURRENT_RATIO_10)
		ratio = CHG_TERM_CURRENT_RATIO_10;
	else if (term_cur_set == SC2721_CHG_TERM_CURRENT_RATIO_20)
		ratio = CHG_TERM_CURRENT_RATIO_20;
	else if (term_cur_set == SC2721_CHG_TERM_CURRENT_RATIO_40)
		ratio = CHG_TERM_CURRENT_RATIO_40;
	else
		ratio = CHG_TERM_CURRENT_RATIO_90;

	*term_cur = info->cur.dcp_cur * ratio / 100;

	dev_info(info->dev, "get_termination_current: CFG0: 0x%x, dcp_cur: %d, real cur: %d, term_cur: %d\n",
			val, info->cur.dcp_cur, chg_cur, *term_cur);

	return 0;
}

static void sc2721_charger_dump_regs(struct sc2721_charger_info *info)
{
	u32 val_ctr0 = 0, val_ctr1 = 0, val_status = 0;
	int ret = 0, chg_cur = 0;

	regmap_read(info->regmap, info->base + SC2721_CHG_CFG0, &val_ctr0);
	regmap_read(info->regmap, info->base + SC2721_CHG_CFG1, &val_ctr1);
	regmap_read(info->regmap, info->base + SC2721_CHG_STATUS, &val_status);
	...
	dev_info(info->dev, "info->charging: %d, info->limit: %d, current: %d\n",
			  info->charging, info->limit, chg_cur);
	dev_info(info->dev, "dump regs: ctr0: 0x%x, ctr1: 0x%x, status: 0x%x\n",
			  val_ctr0, val_ctr1, val_status);
}
...
static int sc2721_charger_hw_init(struct sc2721_charger_info *info)
{
	...
	if (ret) {
		...
		/* Set charge over voltage protection value 6000mv */
		ret = regmap_update_bits(info->regmap,
...
static int sc2721_charger_set_current(struct sc2721_charger_info *info, u32 cur)
{
	...
	if (cur > SC2721_CHG_CURRENT_MAX)
		cur = SC2721_CHG_CURRENT_MAX;
					   ...
					   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = info->charge_vol_max;
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		ret = sc2721_get_termination_current(info, &val->intval);
		if (ret) {
			dev_err(info->dev, "get charge termination current failed\n");
			goto out;
		}
		break;
	default:
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		/* fall through */
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		dev_info(info->dev, "set charge current, psp %d, val = %d\n", psp, val->intval);
	...
	case POWER_SUPPLY_PROP_FEED_WATCHDOG:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		ret = sc2721_set_termination_current(info, val->intval);
		if (ret)
			dev_err(info->dev, "failed to set charge termination current\n");
		break;
	default:
