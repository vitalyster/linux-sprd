		...
		size_t count, loff_t *offset)
{
	...
	pr_info("%s: start reading of %zu bytes\n", __func__, count);
	...
	pr_info("%s: Success in reading %zu bytes\n", __func__, count);
	return ret;
err:
	mutex_unlock(&nfc_dev->read_mutex);
	return ret;
		...
		size_t count, loff_t *offset)
{
	...
	pr_info("%s: start writing of %zu bytes\n", __func__, count);
	...
	pr_info("%s: Success in writing %zu bytes\n", __func__, count);
	/* delay of 1ms for slow devices*/
	udelay(1000);
	return ret;
}
		...
		const struct i2c_device_id *id)
{
	...
	/*zte-modify quguotao test hw begin*/
	...
	/*zte-modify quguotao test hw end*/
