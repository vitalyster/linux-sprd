...
static void sprd_set_delay_value_ums512(struct sprd_sdhc_host *host)
{
	struct device *dev = &host->pdev->dev;
...
static void sprd_sdhc_set_ios_ums512(struct mmc_host *mmc, struct mmc_ios *ios)
{
	struct sprd_sdhc_host *host = mmc_priv(mmc);
	struct device *dev = &host->pdev->dev;
	unsigned long flags;
    ...
    */
	...
	sprd_set_delay_value_ums512(host);
...
static void sprd_sdhc_hs400_enhanced_strobe_ums512(struct mmc_host *mmc,
	struct mmc_ios *ios)
{
	struct sprd_sdhc_host *host = mmc_priv(mmc);
	unsigned long flags;
...
static const struct mmc_host_ops sprd_sdhc_ops_ums512 = {
	.request = sprd_sdhc_request,
	.set_ios = sprd_sdhc_set_ios_ums512,
	.get_cd = sprd_sdhc_get_cd,
	.start_signal_voltage_switch = sprd_sdhc_set_vqmmc,
	.card_busy = sprd_sdhc_card_busy,
	.hw_reset = sprd_sdhc_emmc_hw_reset,
	.prepare_hs400_tuning = sprd_sdhc_prepare_hs400_tuning,
	.execute_tuning = sprd_sdhc_execute_tuning,
	.hs400_enhanced_strobe = sprd_sdhc_hs400_enhanced_strobe_ums512,
};
				...
				struct mmc_host *mmc)
{
	...
	err = of_property_count_strings(np, "sprd,pro-name");
	if (err < 0) {
		mmc->ops = &sprd_sdhc_ops;
		dev_info(dev, "use default ops!!!");
	} else {
		mmc->ops = &sprd_sdhc_ops_ums512;
		dev_info(dev, "only emmc use ums512 ops!!!");
	}
