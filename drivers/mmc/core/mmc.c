...
#define MMC_REV_CMD_FOR_MANU 60
#define BW_GC_CMD_TIMEOUT_MS 100
...
static int emmc_id_proc_show(struct seq_file *m, void *v)
{
	...
	/*zte add emmc fwrev*/
	snprintf(fwrev, sizeof(fwrev), "0x%*phN\n", 8, card->ext_csd.fwrev);
	fwrev[18] = '\0';
	seq_printf(m, "%s-%s-%s-%02d/%04d-%s-NA\n",
					manfname,
					card->cid.prod_name,
					fwrev,
					card->cid.month, card->cid.year,
