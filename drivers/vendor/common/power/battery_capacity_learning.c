...
struct batt_cap_learn_info {
	...
	struct wakeup_source		*bcl_wake_lock;
...
static int batt_cap_learn_battery_begin(struct batt_cap_learn_info *chip)
{
	...
	__pm_stay_awake(chip->bcl_wake_lock);
...
static void batt_cap_learn_probe_work(struct work_struct *work)
{
	...
	chip->bcl_wake_lock = wakeup_source_register(chip->dev, "battery_capacity_learning");
	if (!chip->bcl_wake_lock) {
		pr_alarm("battery_capacity_learning wakelock register failed\n");
		goto register_power_supply_failed;
	}
