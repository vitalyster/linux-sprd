...
#include <linux/io.h>     /*QChen added here*/
...
void read_hardware_register_gpio(unsigned int phy_address)
{
	unsigned long *vir_address;
	unsigned int var_TLMM_GPIO_CFG, var_TLMM_GPIO_IN_OUT, var_TLMM_GPIO_INTR_CFG, var_TLMM_GPIO_INTR_STATUS;

	vir_address = (unsigned long *)ioremap(phy_address, 32);

	pr_err("read_hardware_register_gpio enterv 0x%x qc\n", vir_address);

	var_TLMM_GPIO_CFG = *(vir_address);
	var_TLMM_GPIO_IN_OUT = *(vir_address+4);
	var_TLMM_GPIO_INTR_CFG = *(vir_address+8);
	var_TLMM_GPIO_INTR_STATUS = *(vir_address+12);

	pr_err("#=====================================#");
	pr_err("#TLMM_GPIO_CFG     :%08X\n", var_TLMM_GPIO_CFG);
	pr_err("#TLMM_GPIO_IN_OUT     :%08X\n", var_TLMM_GPIO_IN_OUT);
	pr_err("#TLMM_GPIO_INTR_CFG     :%08X\n", var_TLMM_GPIO_INTR_CFG);
	pr_err("#TLMM_GPIO_INTR_STATUS     :%08X\n", var_TLMM_GPIO_INTR_STATUS);
}
...
static int aw881xx_syspll_check(struct aw881xx *aw881xx)
{
	...
	for (i = 0; i < AW881XX_SYSST_CHECK_MAX; i++) {
		...
		if (ret < 0) {
			aw_dev_dbg(aw881xx->dev, "%s:check pll lock fail, cnt=%d", __func__, i);
...
static int aw881xx_request_pinctrl(struct device *dev)
{
	...
	gpio_state_init = pinctrl_lookup_state(pinctrl, "audio_gpio_init");
	if (IS_ERR(gpio_state_init)) {
		dev_err(dev, "Can't find pinctrl state\n");
		return PTR_ERR(gpio_state_init);
	}
				...
				const struct i2c_device_id *id)
{
	...
	/*  zte add*/
	aw881xx_request_pinctrl(&i2c->dev);
	...
	/* test code
	read_hardware_register_gpio(0xF187000);
	*/
