	...
	unsigned int cmd, void __user *arg)
{
	...
	switch(cmd) {
        ...
        case TFA98XX_DO_CALIBRATE:
			if(tfa98xx_pri->tfa->is_probus_device ==1){
...
static ssize_t tfa98xx_ioctrl_rpc_read_nondsp(struct file *file,
					 char __user *user_buf, size_t count,
					 loff_t *ppos)
{
...
static ssize_t tfa98xx_ioctrl_rpc_read_dsp(struct file *file,
					 char __user *user_buf, size_t count,
					 loff_t *ppos)
{
	struct tfa98xx *tfa98xx_pri = NULL, *tfa98xx_sec = NULL;
	...
	copy_from_user(cmd, user_buf, sizeof(cmd));
	if((cmd[0] == 0x00)&&(cmd[1] ==0x80)&&(cmd[2] ==0x8b))
	{
		pr_err("%s,enter memtrack_94 get\n",__func__);
		buffer = kmalloc(24, GFP_KERNEL);
		
	}else if((cmd[1] ==0x81)&&(cmd[2] ==0x85)){
		pr_err("%s,enter re get\n",__func__);
	...
	if((cmd[0] == 0x00)&&(cmd[1] ==0x80)&&(cmd[2] ==0x8b))
	{
		pr_err("%s,enter memtrack_94 get2\n",__func__);
		temp_1[0] = buffer[6]; //r0
		temp_1[1] = buffer[7];
		temp_1[2] = buffer[8];
		buffer[6] = buffer[15];  
		buffer[7] = buffer[16];
					 ...
					 loff_t *ppos)
{
	...
	if (tfa98xx->tfa->is_probus_device) 
	{
		return tfa98xx_ioctrl_rpc_read_nondsp(file,user_buf,count,ppos);
	}else{
		return tfa98xx_ioctrl_rpc_read_dsp(file,user_buf,count,ppos);
	}
}

static ssize_t tfa98xx_ioctrl_rpc_send_nondsp(struct file *file,
					 const char __user *user_buf,
					 size_t count, loff_t *ppos)
{
	struct tfa98xx *tfa98xx = list_last_entry(&tfa98xx_device_list, struct tfa98xx, list);
...
static ssize_t tfa98xx_ioctrl_rpc_send_dsp(struct file *file,
					 const char __user *user_buf,
					 size_t count, loff_t *ppos)
{
	struct tfa98xx *tfa98xx_pri = NULL, *tfa98xx_sec = NULL;
	...
	int err = 0;
	uint8_t cmd[3] = {0};
	const uint8_t memtrack_94[] = {
		0x00, 0x80, 0x0b,   /*livedata*/
		0x00, 0x00, 0x03,   /*3 items*/
		0x02, 0x07, 0x0b,   /*ReT*/
		0x02, 0x07, 0x05,   /*Tspk*/
		0x02, 0x06, 0xfc    /*fRes*/
	};
	...
	if((cmd[0] == 0x00)&&(cmd[1] ==0x80)&&(cmd[2] ==0x0b))
	{
		pr_err("%s,enter memtrack_94 set\n",__func__);
		msg_file = kmalloc(sizeof(memtrack_94) + sizeof(TfaFileDsc_t), GFP_KERNEL);
		if ( msg_file == NULL ) {
			pr_debug("tfa98xx_ioctrl_rpc_send can not allocate memory\n");
			return	-ENOMEM;
		}
		msg_file->size = sizeof(memtrack_94);
		
		memcpy(msg_file->data, memtrack_94, sizeof(memtrack_94));
	}else{
		/* msg_file.name is not used */
		msg_file = kmalloc(count + sizeof(TfaFileDsc_t), GFP_KERNEL);
		if ( msg_file == NULL ) {
		...
		if((msg_file->data[1] == 0x81) && (msg_file->data[2] == 0x85))
		{
			...
			pr_err("%s,enter re set\n",__func__);
		}else if((msg_file->data[0] == 0x00) && (msg_file->data[1] == 0x80)&& (msg_file->data[2] == 0x3f)){
			pr_err("%s,enter bypass set, do nothing\n",__func__);
			return count;
		}
	}
	mutex_lock(&tfa98xx_pri->dsp_lock);
					 ...
					 size_t count, loff_t *ppos)
{
	...
	if (tfa98xx->tfa->is_probus_device) 
	{
		return tfa98xx_ioctrl_rpc_send_nondsp(file,user_buf,count,ppos);
	}else{
		return tfa98xx_ioctrl_rpc_send_dsp(file,user_buf,count,ppos);
	}
...
//#endif
	...
	if (ret)
		...
		switch (reg & 0xff) {
		case 0x72: /* tfa9872 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9872");
			break;
		case 0x73: /* tfa9873 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9873");
			break;
		case 0x74: /* tfa9874 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9874");
			break;
		case 0x78: /* tfa9878 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9878");
			break;
		case 0x88: /* tfa9888 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9888");
			break;
		case 0x13: /* tfa9912 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9912");
			break;
		case 0x94: /* tfa9894 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9894");
			break;
		case 0x80: /* tfa9890 */
		case 0x81: /* tfa9890 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9890");
			break;
		case 0x92: /* tfa9891 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9891");
			break;
		case 0x12: /* tfa9895 */
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9895");
			break;
		case 0x97:
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9897");
			break;
		case 0x96:
			tfa98xx_set_vendor_info(VENDOR_GOODIX, "tfa9896");
			break;
		default:
			pr_info("Unsupported device revision (0x%x)\n", reg & 0xff);
			break;
