...
int ili_sleep_handler(int mode)
{
	...
	case TP_SUSPEND:
		...
		if (ilits->gesture) {
			...
			ili_irq_enable();
			tpd_fw_cdev.ignore_tp_irq = true;
			ILI_INFO("ignore tp irq until lcd suspend finished\n");
		} else {
	...
	case TP_RESUME:
		...
		if (tpd_fw_cdev.ignore_tp_irq) {
			tpd_fw_cdev.ignore_tp_irq = false;
			ILI_INFO("set ignore_tp_irq false\n");
		}
		if (!ilits->tp_suspend) {
			ili_wq_ctrl(WQ_ESD, ENABLE);
			ili_wq_ctrl(WQ_BAT, ENABLE);
