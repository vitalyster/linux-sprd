...
#define CSV_FILE_SIZE		(512 * K)

#define PARSER_MAX_CFG_BUF		512
#define PARSER_MAX_KEY_NUM		1000
#define PARSER_MAX_KEY_NAME_LEN		100
#define PARSER_MAX_KEY_VALUE_LEN	512
...
static struct mp_test_items tItems[MP_TEST_ITEM] = {
	...
	{.desp = "untouch signal data(bg-raw-4096) - mutual", .catalog = MUTUAL_TEST,
		.cmd = CMD_MUTUAL_SIGNAL, .lcm = ON},
...
static void parser_ini_nodetype(s32 *type_ptr, char *desp, int frame_len)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		if (!strnstr(ini_info[i].section_name, desp, PARSER_MAX_KEY_NAME_LEN) ||
	...
	while (true) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				banchmark_flag = 0;
				nodetype_flag = 0;
				strlcpy(seq_item[scount], tmpSectionName, PARSER_MAX_KEY_NAME_LEN);
				scount++;
		...
		strlcpy(ini_info[g_ini_items].section_name, tmpSectionName, (PARSER_MAX_KEY_NAME_LEN * sizeof(char)));
		...
		if (isEqualSign == 0) {
			...
			if (strnstr(ini_info[g_ini_items].section_name, BENCHMARK_KEY_NAME,
				PARSER_MAX_KEY_NAME_LEN) != NULL) {
				isEqualSign = -1;
				ini_info[g_ini_items].key_name_len = strlen(ini_info[g_ini_items].section_name);
				strlcpy(ini_info[g_ini_items].key_name, ini_info[g_ini_items].section_name,
					(PARSER_MAX_KEY_NAME_LEN * sizeof(char)));
				ini_info[g_ini_items].key_val_len = n;
			} else if (strnstr(ini_info[g_ini_items].section_name, NODE_TYPE_KEY_NAME,
			PARSER_MAX_KEY_NAME_LEN) != NULL) {
				isEqualSign = -1;
				ini_info[g_ini_items].key_name_len = strlen(NODE_TYPE_KEY_NAME);
				strlcpy(ini_info[g_ini_items].key_name, NODE_TYPE_KEY_NAME,
					(PARSER_MAX_KEY_NAME_LEN * sizeof(char)));
				ini_info[g_ini_items].key_val_len = n;
			} else {
				continue;
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line, int file_size)
{
	...
	tmp_line++;
	tmp_len += snprintf(csv + tmp_len, (file_size - tmp_len), "========================================n");
	tmp_line++;
...
static void mp_print_csv_cdc_cmd(char *csv, int *csv_len, int index, int file_size)
{
	...
	if (ipio_strcmp(name, "open test(integration)_sp") == 0) {
		...
		for (i = 0; i < size; i++) {
			slen = parser_get_int_data("pv5_4 command", (char *)open_sp_cmd[i], str, sizeof(str));
			if (slen < 0)
		...
		for (i = 0; i < size; i++) {
			slen = parser_get_int_data("pv5_4 command", (char *)open_c_cmd[i], str, sizeof(str));
			if (slen < 0)
				ILI_ERR("Failed to get CDC command %s from ini\n", open_sp_cmd[i]);
			else
				tmp_len += snprintf(csv + tmp_len, (file_size - tmp_len),
...
static s32 open_c_formula(int inCap1DAC, int inCap1Raw, int accuracy)
{
	...
	int inVadc_range = 36;
	int inVbk = 39;
	int inCbk_step = open_para.cbk_step;
	int inCint = open_para.cint;
	int inVdrv = open_para.tvch - open_para.tvcl;
	int inGain = open_para.gain;
	int inJudge_Para = 0;
	int inMagnification = 10;
	int inPartDac = 0;
	int inPartRaw = 0;

	inJudge_Para = inCbk_step * inCint * inVdrv * inGain;
	if (inJudge_Para == 0) {
		if (id == ILI9881_CHIP) {
	...
	if ((id == ILI7807_CHIP) && (type == ILI_S)) {
		inVbk = 42;
		inMagnification = 1;
		if (accuracy) {
			inCap1Value = ((inPartDac + inPartRaw) * 100) / inVdrv / inMagnification / inGain;
		} else {
...
static int codeToOhm(s32 *ohm, s32 *Code, u16 *v_tdf, u16 *h_tdf)
{
	...
	int inTDF1 = 0;
	int inTDF2 = 0;
	int inCint = short_para.cint;//the same of open
	...
	inJudge_Para = inTVCH * inTVCL * inCint * inVariation * inRinternal;
	if (inJudge_Para == 0) {
		inVariation = 100;
		if (id == ILI9881_CHIP) {
			if ((type == ILI_N) || (type == ILI_O)) {
...
static int short_test(int index, int frame_index)
{
	...
	inJudge_Para = short_para.variation * short_para.cint * short_para.rinternal;
	if (inJudge_Para == 0) {
		ILI_INFO("[mp setting][no short paramter] Fill in Default Parameter");
	}

	ILI_INFO("TVCH = %d, TVCL = %d, Variation = %d,\n"
		, short_para.tvch, short_para.tvcl, short_para.variation);
	ILI_INFO("V_TDF1 = %d, V_TDF2 = %d, H_TDF1 = %d, H_TDF2 = %d, Cint = %d, Rinternal = %d\n"
...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	u8 *ori = NULL;
	char str[128] = {0};
	static char const *key[] = {"open dac", "open raw1", "open raw2", "open raw3",
	...
	if (parser_get_int_data("pv5_4 command", (char *)key[mode], str, sizeof(str)) < 0) {
...
static int allnode_peak_to_peak_cdc_data(int index)
{
	...
	if (frm_buf == NULL) {
		frm_buf = kzalloc((tItems[index].bch_mrk_frm_num * sizeof(s32 *)), GFP_KERNEL);
		...
		for (i = 0 ; i < tItems[index].bch_mrk_frm_num ; i++) {
			frm_buf[i] = kzalloc((core_mp.frame_len * sizeof(s32)), GFP_KERNEL);
			if (ERR_ALLOC_MEM(frm_buf)) {
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].bch_mrk_max == NULL) {
				tItems[index].bch_mrk_max =
					kzalloc(tItems[index].bch_mrk_frm_num * sizeof(s32 *), GFP_KERNEL);
				...
				for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
					tItems[index].bch_mrk_max[i] =
						kzalloc(core_mp.frame_len * sizeof(s32), GFP_KERNEL);
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (i == 0) {
			ipio_memcpy(frame1_cbk700, open[i].cbk_700,
				core_mp.frame_len * sizeof(s32), core_mp.frame_len * sizeof(s32));
		...
		ili_dump_data(&tItems[index].buf[(i * core_mp.frame_len)], 10,
			core_mp.frame_len, core_mp.xch_len, "after compare charge rate");
...
static int open_test_cap(int index)
{
	...
	inJudge_Para = open_para.cbk_step * open_para.cint * (open_para.tvch - open_para.tvcl) * open_para.gain;
	if (inJudge_Para == 0) {
		ILI_INFO("[mp setting][no open_c paramter] Fill in Default Parameter");
	}
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].trimmed_mean && tItems[index].catalog != PEAK_TO_PEAK_TEST) {
			...
			mp_compare_cdc_result(index, tItems[index].result_buf,
				max_threshold, min_threshold, &test_result);
		} else {
			if (tItems[index].bch_mrk_multi) {
				for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
					mp_compare_cdc_result(index, frm_buf[i],
					tItems[index].bch_mrk_max[i], tItems[index].bch_mrk_min[i], &test_result);
				}
			} else {
				mp_compare_cdc_result(index, tItems[index].max_buf,
					max_threshold, min_threshold, &test_result);
				mp_compare_cdc_result(index, tItems[index].min_buf,
					max_threshold, min_threshold, &test_result);
			}
		}
	}
...
static int mp_show_result(bool lcm_on)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
			...
			if (tItems[i].catalog == PEAK_TO_PEAK_TEST) {
				...
				for (j = 0; j < tItems[i].bch_mrk_frm_num; j++) {
					...
					mp_compare_cdc_show_result(i, tItems[i].bch_mrk_max[j], csv, &csv_len,
						TYPE_BENCHMARK, max_threshold, min_threshold, bench_name,
						CSV_FILE_SIZE);
				...
				mp_compare_cdc_show_result(i, tItems[i].bench_mark_max, csv, &csv_len,
					TYPE_BENCHMARK, max_threshold, min_threshold, "Max_Bench", CSV_FILE_SIZE);
				mp_compare_cdc_show_result(i, tItems[i].bench_mark_min, csv, &csv_len,
					TYPE_BENCHMARK, max_threshold, min_threshold, "Min_Bench", CSV_FILE_SIZE);
			}
		...
		if (ipio_strcmp(tItems[i].desp, "open test(integration)_sp") == 0) {
			mp_compare_cdc_show_result(i, frame1_cbk700, csv, &csv_len,
				TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk700", CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, frame1_cbk250, csv, &csv_len,
				TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk250", CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, frame1_cbk200, csv, &csv_len,
				TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk200", CSV_FILE_SIZE);
		}

		if (ipio_strcmp(tItems[i].desp, "open test_c") == 0) {
			mp_compare_cdc_show_result(i, cap_dac, csv, &csv_len, TYPE_NO_JUGE,
				max_threshold, min_threshold, "CAP_DAC", CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, cap_raw, csv, &csv_len, TYPE_NO_JUGE,
				max_threshold, min_threshold, "CAP_RAW", CSV_FILE_SIZE);
		}
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			mp_compare_cdc_show_result(i, core_mp.tx_max_buf, csv, &csv_len,
				TYPE_JUGE, max_threshold, min_threshold, "TX Max Hold", CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, core_mp.tx_min_buf, csv, &csv_len,
				TYPE_JUGE, max_threshold, min_threshold, "TX Min Hold", CSV_FILE_SIZE);

			for (j = 0; j < core_mp.frame_len; j++) {
				max_threshold[j] = core_mp.RxDeltaMax;
				min_threshold[j] = core_mp.RxDeltaMin;
			}
			mp_compare_cdc_show_result(i, core_mp.rx_max_buf, csv, &csv_len,
				TYPE_JUGE, max_threshold, min_threshold, "RX Max Hold", CSV_FILE_SIZE);
			mp_compare_cdc_show_result(i, core_mp.rx_min_buf, csv, &csv_len,
				TYPE_JUGE, max_threshold, min_threshold, "RX Min Hold", CSV_FILE_SIZE);
		} else {
			/* general result */
			if (tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST) {
				mp_compare_cdc_show_result(i, tItems[i].result_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold, "Mean result", CSV_FILE_SIZE);
			} else {
				if (!tItems[i].bch_mrk_multi) {
					mp_compare_cdc_show_result(i, tItems[i].max_buf, csv, &csv_len,
						TYPE_JUGE, max_threshold, min_threshold, "Max Hold", CSV_FILE_SIZE);
					mp_compare_cdc_show_result(i, tItems[i].min_buf, csv, &csv_len,
						TYPE_JUGE, max_threshold, min_threshold, "Min Hold", CSV_FILE_SIZE);
				}
			...
			for (j = 0; j < get_frame_cont; j++) {
				...
				mp_compare_cdc_show_result(i, &tItems[i].buf[(j*core_mp.frame_len)],
					csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, frame_name,
					CSV_FILE_SIZE);
			}
		}
	}
	...
	if (pass_item_count == 0) {
		...
		ret = MP_DATA_FAIL;
		if (lcm_on)
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s.csv",
			ilits->md_save_file_path, get_date_time_str(), ret_fail_name);
		else
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_off.csv",
			ilits->md_save_file_path, get_date_time_str(), ret_fail_name);
	} else {
		...
		if (lcm_on)
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s.csv",
			ilits->md_save_file_path, get_date_time_str(), ret_pass_name);
		else
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_off.csv",
			ilits->md_save_file_path, get_date_time_str(), ret_pass_name);
	}
	...
	if (tItems[i].catalog == PEAK_TO_PEAK_TEST) {
		if (tItems[i].max_min_mode == 1 && tItems[i].spec_option == 1) {
			...
			for (j = 1; j < 11; j++) {
				char tmp[8] = {0};
				/* format complete string from the name of section "_Benchmark_Data". */
				snprintf(str, sizeof(str), "%s%s%s%d", tItems[index].desp, "_",
...
static void mp_copy_ret_to_apk(char *buf)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		if (tItems[i].item_result == MP_DATA_FAIL) {
			...
			if (strnstr( tItems[i].desp, "short", strlen(tItems[i].desp))) {
					ilitek_tptest_result = ilitek_tptest_result | TEST_GT_SHORT;
			} else if (strnstr( tItems[i].desp, "open", strlen(tItems[i].desp))) {
					ilitek_tptest_result = ilitek_tptest_result | TEST_GT_OPEN;
			} else {
					ilitek_tptest_result = ilitek_tptest_result
						| TEST_BEYOND_MAX_LIMIT | TEST_BEYOND_MIN_LIMIT;
			}
		} else {
			ILI_INFO("[%s] = PASS\n", tItems[i].desp);
...
int ili_mp_test_main(char *apk, bool lcm_on)
{
	...
	ini_info = vmalloc(sizeof(struct ini_file_data) * PARSER_MAX_KEY_NUM);
