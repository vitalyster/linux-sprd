...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	cdev->ic_tpinfo.module_id = (g_tlsc6x_cfg_ver >> 9) & 0x7f;
...
{
	tlsc_info("%s:enter\n", __func__);
	tlsc6x_test_result_node = kzalloc(MAX_NODE_SIZE, GFP_KERNEL);
...
{
	if ((tlsc6x_test_result_node == NULL) || (failed_node >= MAX_NODE_SIZE)) {
		return -EPERM;
	}
