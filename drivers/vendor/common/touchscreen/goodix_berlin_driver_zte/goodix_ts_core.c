...
int __goodix_set_edge_suppress_gt9897(int *cmd_start, struct goodix_ts_core *core_data)
{
	struct goodix_ext_module *ext_module, *next;
	int ret = -1;
...
static int goodix_set_edge_suppress_gt9897(int level, int is_hor, void *priv_data)
{
	...
	if (is_hor & 0x02) {
		...
		ret = __goodix_set_edge_suppress_gt9897(&(ts_bdata->edge_suppress_cmd[GOODIX_CMD_LEN * level]),
									core_data);
	} else {
		...
		__goodix_set_edge_suppress_gt9897(
			&(ts_bdata->edge_suppress_cmd[GOODIX_CMD_LEN * DEFAULT_SUPPRESS_LEVEL]),
									core_data);
	}
