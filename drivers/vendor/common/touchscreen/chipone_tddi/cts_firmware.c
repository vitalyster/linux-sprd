...
static u32 crc32_new(const u8 *data, size_t len)
{
			...
			char *filepath, u16 curr_version)
{
	...
	/*-------FWID CHECKED----*/
	fwid =  *((u16 *)(fw->data + FIRMWARE_FS_FWID));
	cts_info("'FS_FWID: %04x",  fwid);

	if (fwid != cts_dev->hwdata->fwid) {
		cts_err("FWID: %04x != %04x, dont need upgrade.", fwid, cts_dev->hwdata->fwid);
		goto err_free_fw;
	}
...
/*-------FWID CHECKED----*/
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	offset = FIRMWARE_FS_FWID;
	if (kernel_read(file, buff, 2, &offset) < 0) {
		cts_err("Read FS_FWID from offset 0xC8 failed");
		goto err_close_file;
	}
#else
	if (kernel_read(file, FIRMWARE_FS_FWID, buff, 2) < 0) {
		cts_err("Read FS_FWID from offset 0xC8 failed");
		goto err_close_file;
	}
#endif
	fwid = get_unaligned_le16(buff);

	cts_info("File '%s' FS_FWID: %04x", filepath,  fwid);

	if (fwid != cts_dev->hwdata->fwid) {
		cts_info("File '%s'  FWID: %04x != %04x", filepath,  fwid, cts_dev->hwdata->fwid);
		goto err_close_file;
	}
...
int cts_update_firmware(struct cts_device *cts_dev, const struct cts_firmware *firmware, bool to_flash)
{
	...
	if (!to_flash) {
		...
		if (cts_dev->hwdata->hwid == CTS_HWID_ICNL9911S ||
			cts_dev->hwdata->hwid == CTS_HWID_ICNL9911C) {
