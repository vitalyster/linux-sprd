	...
	size_t len, u32 crc, int retry)
{
	...
	retries = 0;
	do {
		if ((ret = cts_i2c_writel(cts_dev, 0x015ff0, 0xCC33555A, 3, 1)) != 0) {
			cts_err("SRAM writesb failed %d", ret);
			continue;
		}

		if ((ret = cts_i2c_writel(cts_dev, 0x08fffc, crc, 3, 1)) != 0) {
			cts_err("SRAM writesb failed %d", ret);
			continue;
		}

		if ((ret = cts_i2c_writel(cts_dev, 0x08fff8, len, 3, 1)) != 0) {
			cts_err("SRAM writesb failed %d", ret);
			continue;
		}
...
int cts_enter_normal_mode(struct cts_device *cts_dev)
{
	...
	if(ret){
		...
		if(fwid==CTS_FWID_ICNL9911 || fwid==CTS_FWID_ICNL9911S || fwid == CTS_FWID_ICNL9911C)
		{
