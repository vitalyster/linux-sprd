...
{
	if (!tpd_fw_cdev.ignore_tp_irq) {
		fts_irq_read_report();
	} else {
		FTS_INFO("lcd  reset, irq return");
	}
	return IRQ_HANDLED;
}
...
#ifndef FTS_SUSPEND_WITHOUT_POWER_CTRL
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ts_data->rbuf = kmalloc(I2C_RBUF_SIZE, GFP_KERNEL);
	if (ts_data->rbuf == NULL) {
	...
	if (ret < 0)
		...
		FTS_INFO("FTS tp register adf notifier succeed\n");
#elif defined(CONFIG_DRM_MSM)
	ts_data->fb_notif.notifier_call = drm_notifier_callback;
	ret = msm_drm_register_client(&ts_data->fb_notif);
	if (ret) {
		FTS_ERROR("[FB]Unable to register drm_notifier: %d", ret);
	}
#elif defined(CONFIG_FB)
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret =  get_tp_chip_id();
	if (ret == 0) {
		if ((tpd_fw_cdev.tp_chip_id != TS_CHIP_MAX) && (tpd_fw_cdev.tp_chip_id != TS_CHIP_FOCAL)) {
