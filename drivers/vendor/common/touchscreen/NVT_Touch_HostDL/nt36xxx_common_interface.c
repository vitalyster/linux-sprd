...
#define FW_TP_STATUS_BUFFER (64 * 2 + 2)
...
#ifdef NVT_PINCTRL_EN
#define NVT_PINCTRL_INIT_STATE "nvt_gpio_init"
#endif
...
char nt36xx_default_firmware_name[MAX_FILE_NAME_LEN] = {0};
...
#ifdef NVT_PINCTRL_EN
int nvt_platform_pinctrl_init(void)
{
	int ret = 0;
	...
	if (ret < 0) {
		NVT_ERR("failed to select pin to init state");
...
out:
	...
	snprintf( nt36xx_default_firmware_name, sizeof(nt36xx_default_firmware_name),
			"%s_%s.bin", NVT_DEFAULT_FIRMWARE, nt36xx_vendor_name);
	return ret;
}
...
int nt36xx_cmd_ctrl(enum nvt_tp_cmd_type cmd, uint8_t ctrl)
{
	...
	case CHARGER:
		...
		buf[2] = ctrl ? 1 : 0;
		NVT_LOG("charger cmd, set %d.\n", ctrl);
		break;
	case SENSIBILITY:
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = EVENT_MAP_SENSIBILITY;
		buf[2] = ctrl;
		NVT_LOG("sensibility cmd, set %d.\n", ctrl);
		break;
	default:
		NVT_ERR("Invalid cmd!\n");
		return -EINVAL;
...
static int nt36xx_set_sensibility_level(struct tpd_classdev_t *cdev, u8 level)
{
	ts->sensibility_level = level;
	cdev->sensibility_level = ts->sensibility_level;
	NVT_LOG("%s:sensibility = %d.\n", __func__, ts->sensibility_level);
	if (!ts->tp_suspend) {
		if (nt36xx_cmd_ctrl(SENSIBILITY, level) < 0)
			NVT_ERR("%s:Write sensibility failed\n",  __func__);
		else
			NVT_LOG("%s: Write sensibility successful.\n", __func__);
	}
...
void nt36xx_ex_mode_recovery(void)
{
	...
	nt36xx_cmd_ctrl(SENSIBILITY, ts->sensibility_level);
}

#if NVT_TOUCH_WDT_RECOVERY
static bool nt36xx_esd_check(struct tpd_classdev_t *cdev)
{
	if (ts->tp_suspend)
		return 0;
	if (ts->esd_check_error)
		NVT_LOG("%s:esd check error");

	return ts->esd_check_error;
}
#endif
		 ...
		 s16 *frame_data_words, int idex, int idx, char *name)
{
	...
	for (y = 0; y < rows; y++) {
		...
		for (x = 0; x < cols; x++) {
			count += snprintf(buff_arry[idex] + count, RT_DATA_LEN - count,
						"%4d,", frame_data_words[y * cols + x]);
		}
		count += snprintf(buff_arry[idex] + count, RT_DATA_LEN - count, "\n");
	}
	if (strnstr(name, "Rawdata", strlen(name))) {
		count += snprintf(buff_arry[idex] + count, RT_DATA_LEN - count, "History1 Event:");
		for (x = 0; x < FW_TP_STATUS_BUFFER; x++) {
			if (x % 16 == 0) {
				count += snprintf(buff_arry[idex] + count, RT_DATA_LEN - count, "\n");
			}
			count += snprintf(buff_arry[idex] + count, RT_DATA_LEN - count,
						"0x%02x,", frame_data_words[cols * rows + x]);
			if (x == 63)
				count += snprintf(buff_arry[idex] + count, RT_DATA_LEN - count, "\nHistory2 Event:");
			if (x == 127)
				count += snprintf(buff_arry[idex] + count, RT_DATA_LEN - count, "\nFW Status:");
...
static void nvt_ts_read_history_log(uint8_t *buffer, int buffer_len)
{
	uint8_t buf[65] = {0};
	int32_t i = 0;
	...
	if (buffer_len < 130)
		return;
	NVT_LOG("Read Start\n");
	nvt_set_page(0x23D00);
	memset(buf, 0, sizeof(buf));
	buf[0] = 0x10;
	CTP_SPI_READ(ts->client, buf, 64);
	NVT_LOG("History1 Event:\n");
	memcpy(buffer, buf + 1, 64);
	for (i = 0; i <8; i++) {
		NVT_LOG("%02X %02X %02X %02X %02X %02X %02X %02X\n",
			buf[1+i*8], buf[2+i*8], buf[3+i*8], buf[4+i*8], buf[5+i*8], buf[6+i*8], buf[7+i*8], buf[8+i*8]);
	}
	memset(buf, 0, sizeof(buf));
	buf[0] = 0x50;
	CTP_SPI_READ(ts->client, buf, 64);
	NVT_LOG("History2 Event:\n");
	memcpy(buffer + 64, buf + 1, 64);
	for (i = 0; i <8; i++) {
		NVT_LOG("%02X %02X %02X %02X %02X %02X %02X %02X\n",
			buf[1+i*8], buf[2+i*8], buf[3+i*8], buf[4+i*8], buf[5+i*8], buf[6+i*8], buf[7+i*8], buf[8+i*8]);
	}
...
static int nt36xx_data_request(s16 *frame_data_words, enum tp_test_type  test_type)
{
	...
	if (test_type == RAWDATA_TEST) {
		tp_status_buffer =  kzalloc((FW_TP_STATUS_BUFFER + 1), GFP_KERNEL);
		nvt_ts_read_history_log(tp_status_buffer, FW_TP_STATUS_BUFFER);
		for (i = 0; i < FW_TP_STATUS_BUFFER; i++) {
			frame_data_words[index + 1 + i] = tp_status_buffer[i];
		}
	}
...
static int  nt36xx_testing_delta_raw_report(char *buff_arry[], unsigned int num_of_reports)
{
	...
	row = ts->y_num;
	col = ts->x_num;
	frame_data_words = kcalloc((row * col + FW_TP_STATUS_BUFFER + 1), sizeof(s16), GFP_KERNEL);
...
int nt36xx_register_fw_class(void)
{
	...
	tpd_fw_cdev.set_sensibility_leve = nt36xx_set_sensibility_level;
#if NVT_TOUCH_WDT_RECOVERY
	tpd_fw_cdev.tpd_esd_check = nt36xx_esd_check;
#endif
	ts->sensibility_level = 1;
