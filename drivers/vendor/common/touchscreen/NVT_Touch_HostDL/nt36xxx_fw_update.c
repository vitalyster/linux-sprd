		...
		if (list < ilm_dlm_num) {
			...
			if (ts->hw_crc)
				...
				if ((bin_map[list].BIN_addr + bin_map[list].size) < fwsize)
					bin_map[list].crc =
					    CheckSum(&fwdata[bin_map[list].BIN_addr], bin_map[list].size);
				else {
...
static void nvt_set_bld_hw_crc(void)
{
	...
	nvt_set_bld_crc_bank(ts->mmap->ILM_DES_ADDR, bin_map[0].SRAM_addr,
			     ts->mmap->ILM_LENGTH_ADDR, bin_map[0].size, ts->mmap->G_ILM_CHECKSUM_ADDR, bin_map[0].crc);

	/* [1] DLM */
	/* write register bank */
	nvt_set_bld_crc_bank(ts->mmap->DLM_DES_ADDR, bin_map[1].SRAM_addr,
			     ts->mmap->DLM_LENGTH_ADDR, bin_map[1].size, ts->mmap->G_DLM_CHECKSUM_ADDR, bin_map[1].crc);
}
...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	if (ret) {
		NVT_ERR("%s firmware_request failed. try requeset %s\n", firmware_name, DEFAULT_UPDATE_FIRMWARE_NAME);
		ret = update_firmware_request(DEFAULT_UPDATE_FIRMWARE_NAME);
		if (ret) {
			NVT_ERR("%s firmware_request failed.\n", DEFAULT_UPDATE_FIRMWARE_NAME);
