...
#endif
extern void nt36xx_ex_mode_recovery(void);
extern int nt36xx_register_fw_class(void);
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		ts->esd_check_error = false;
	}

	queue_delayed_work(nvt_esd_check_wq, &nvt_esd_check_work, msecs_to_jiffies(NVT_TOUCH_ESD_CHECK_PERIOD));
}
#endif /* #if NVT_TOUCH_ESD_PROTECT */
...
/*
	for (i = 0; i < 10; i++) {
		printk("%02X %02X %02X %02X %02X %02X  ",
		       point_data[1 + i * 6], point_data[2 + i * 6], point_data[3 + i * 6], point_data[4 + i * 6],
	...
	*/
	...
	for (i = 0; i < ts->max_touch_num; i++) {
...
#endif /* #if NVT_TOUCH_ESD_PROTECT */
			input_x =
			    (uint32_t) (point_data[position + 1] << 4) + (uint32_t) (point_data[position + 3] >> 4);
			input_y =
			    (uint32_t) (point_data[position + 2] << 4) + (uint32_t) (point_data[position + 3] & 0x0F);
		...
		tpd_touch_press(ts->input_dev, input_x, input_y, input_id - 1,  input_w, 0);
#else
#if MT_PROTOCOL_B
			press_id[input_id - 1] = 1;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts->xbuf = kzalloc((NVT_TRANSFER_LEN + 1 + DUMMY_BYTES), GFP_KERNEL);
	if (ts->xbuf == NULL) {
...
int32_t nvt_ts_suspend(struct device *dev)
{
	...
	if (ts->gesture) {
		...
		ts->gesture_enter = true;
	} else {
		/* ---write command to enter "deep sleep mode"--- */
		buf[0] = EVENT_MAP_HOST_CMD;
		buf[1] = 0x11;
		CTP_SPI_WRITE(ts->client, buf, 2);
		ts->gesture_enter = false;
	}
#else /* WAKEUP_GESTURE */
	/* ---write command to enter "deep sleep mode"--- */
	buf[0] = EVENT_MAP_HOST_CMD;
