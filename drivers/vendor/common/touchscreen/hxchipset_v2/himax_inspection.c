...
static int himax_chip_self_test(struct seq_file *s, void *v)
{
	...
	I("%s: g_file_path is%s!\n", __func__, g_file_path);
	raw_file = filp_open(g_file_path,  O_TRUNC | O_CREAT | O_RDWR, 0);
	if (IS_ERR(raw_file)) {
	...
	g_core_fp.fp_0f_op_file_dirly(i_CTPM_firmware_name);
	...
	himax_tpd_test_result_check(ret);
