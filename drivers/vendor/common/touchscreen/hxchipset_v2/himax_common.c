...
#else
#ifdef HX_ZERO_FLASH
		I("%s: request firmware %s fail, try request firmware: %s.\n",
			__func__,  i_CTPM_firmware_name,  himax_default_firmware_name);
		ret = request_firmware(&hxfw, himax_default_firmware_name,  private_ts->dev);
		if (ret < 0)
#endif
		{
