...
static bool himax_edge_limit_level_set(uint8_t level)
{
 ...
 static bool himax_sensibility_level_set(uint8_t level)
{
	uint8_t tmp_data[DATA_LEN_4] = { 0 };
	int ret = -1;
	uint32_t sensibility_level_pwd = fw_func_sensibility_level1_pwd;

	if (level > 3)
		level = 3;
	switch (level) {
	case 1:
		sensibility_level_pwd = fw_func_sensibility_level1_pwd;
		break;
	case 2:
		sensibility_level_pwd = fw_func_sensibility_level2_pwd;
		break;
	case 3:
		sensibility_level_pwd = fw_func_sensibility_level3_pwd;
		break;
	default:
		sensibility_level_pwd = fw_func_sensibility_level1_pwd;
		break;
	}
	himax_parse_assign_cmd(sensibility_level_pwd, tmp_data, 4);
	ret = g_core_fp.fp_register_write(pfw_op->addr_sensibility_level, DATA_LEN_4, tmp_data, 0);
	if (ret < 0) {
		I("%s: edge limit level writer fail!\n", __func__);
		return false;
	}
...
#ifndef CONFIG_TOUCHSCREEN_HIMAX_SPI
	uint8_t retry = 0;
...
static void himax_mcu_resend_cmd_func(bool suspended)
{
	...
	if (g_core_fp.fp_headset_mode_set(ts->headset_state, ts->suspended) == true) {
		...
		E("%s: headset write fail/n", __func__);
	}
#endif
#ifdef HX_EDGE_LIMIT
	if (g_core_fp.fp_edge_limit_level_set(ts->edge_limit_level) == true) {
		I("%s: edge_limit_level write success/n", __func__);
	} else {
		E("%s: edge_limit_level write fail/n", __func__);
	}
#endif
#ifdef HX_SENSIBILITY
...
#endif
#ifdef HX_SENSIBILITY
	g_core_fp.fp_set_sensibility_level = himax_sensibility_level_set;
#endif
...
#endif
#ifdef HX_SENSIBILITY
	himax_parse_assign_cmd(fw_sensibility_level_addr,
		pfw_op->addr_sensibility_level, sizeof(pfw_op->addr_sensibility_level));
#endif
#if defined(HX_ULTRA_LOW_POWER)
