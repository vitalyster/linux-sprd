...
void himax_tp_irq_wake(bool enable)
{
	...
	if (enable  && (atomic_read(&ts->irq_wake_enabled) == 0)) {
		enable_irq_wake(irqnum);
		atomic_set(&ts->irq_wake_enabled, 1);
		I("enable_irq_wake");
	} else if (!enable && (atomic_read(&ts->irq_wake_enabled) == 1)) {
		disable_irq_wake(irqnum);
		atomic_set(&ts->irq_wake_enabled, 0);
		I("disable_irq_wake");
	}
}
EXPORT_SYMBOL(himax_tp_irq_wake);
...
int himax_chip_common_probe(struct spi_device *spi)
{
	...
	g_read_xfer_data = kzalloc(sizeof(uint8_t) * (HX_MAX_READ_SZ + 2),
		GFP_KERNEL);
	...
	if (ret < 0) {
		E("Failed to initialized SPI setup\n");
		goto err_spi_setup;
	}
#ifdef CONFIG_SPI_MT65XX
