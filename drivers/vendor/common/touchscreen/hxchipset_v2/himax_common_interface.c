...
int himax_platform_pinctrl_init(struct himax_i2c_platform_data *pdata)
{
	...
	pdata->ts_pinctrl = devm_pinctrl_get(&(private_ts->spi->dev));
#else
	pdata->ts_pinctrl = devm_pinctrl_get(&(private_ts->client->dev));
#endif
...
int himax_get_fw_by_lcminfo(void)
{
	...
	if (HXTS_MODULE_NUM == 1) {
		...
		snprintf(himax_default_firmware_name, sizeof(himax_default_firmware_name),
			"%s_%s.bin", HXTS_DEFAULT_FIRMWARE, HXTS_VENDOR_0_NAME);
		snprintf(himax_mp_firmware_name, sizeof(himax_mp_firmware_name),
			"Himax_mp_firmware_%s.bin", HXTS_VENDOR_0_NAME);
#endif
		return 0;
	}

	panel_name = get_lcd_panel_name();
	I("himax panel_name :%s\n", panel_name);
...
#ifdef HX_ZERO_FLASH
			snprintf(himax_default_firmware_name, sizeof(himax_default_firmware_name),
				"%s_%s.bin", HXTS_DEFAULT_FIRMWARE, himax_vendor_l[i].vendor_name);
			snprintf(himax_mp_firmware_name, sizeof(himax_mp_firmware_name),
...
static int himax_set_sensibility_level(struct tpd_classdev_t *cdev, u8 level)
{
	struct himax_ts_data *ts = private_ts;
...
#ifdef HX_SENSIBILITY
	tpd_fw_cdev.set_sensibility_leve = himax_set_sensibility_level;
	private_ts->sensibility_level = 1;
#endif
#ifdef HX_LCD_OPERATE_TP_RESET
