...
static void egis_report_uevent(struct egistec_data *data, char *str)
{
	...
	if (data->pnavdev) {
		egislog_d("send uevent %s\n", str);
		kobject_uevent_env(&(data->pnavdev->dev.kobj), KOBJ_CHANGE, envp);
	} else {
		egislog_e("pnavdev is null!\n");
	}
}
...
static void send_key_event(struct egistec_data *egistec, unsigned int code, int value)
{
	...
	egislog_d("Egis navigation driver, send key event: %d, action: %d\n", code, value);
}
...
{
	egislog_d("Egis navigation driver, translated cmd: %d\n", cmd);
	...
	case NAVI_EVENT_OFF: /* finger up */
		if (g_KeyEventRaised == false) {
			g_KeyEventRaised = true;
			egislog_i("Egis : g_SingleClick %u tap interval =%u double tap interval = %u time= %u",
...
#ifdef ZTE_NAV_UEVENT
			egis_report_uevent(egistec,FP_NAV_UP);
#endif
		if (g_KeyEventRaised == false) {
			g_KeyEventRaised = true;
...
#ifdef ZTE_NAV_UEVENT
			egis_report_uevent(egistec,FP_NAV_DOWN);
#endif
		if (g_KeyEventRaised == false) {
			g_KeyEventRaised = true;
#if ENABLE_SWIPE_UP_DOWN
...
#ifdef ZTE_NAV_UEVENT
			egis_report_uevent(egistec,FP_NAV_RIGHT);
#endif
...
#ifdef ZTE_NAV_UEVENT
			egis_report_uevent(egistec,FP_NAV_LEFT);
#endif
		if (g_KeyEventRaised == false) {
			g_KeyEventRaised = true;
	...
	default:
		egislog_e("Egis navigation driver, cmd not match\n");
	}
}
...
{
	egislog_d("Egis navigation driver, straight cmd: %d\n", cmd);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	egislog_d("Egis navigation driver echo :'%d'\n", *buf);
	...
	if (egistec) {
		...
		egislog_e("Egis navigation driver, egistec is NULL\n");

	if (egistec->input_dev == NULL)
		egislog_e("Egis navigation driver, egistec->input_dev is NULL\n");
	...
	if (tempcmd != NULL) {
		...
		egislog_e("navigation_event_func kmalloc failed\n");
...
{
	struct egistec_data *egistec = dev_get_drvdata(dev);
	egislog_e("Egis navigation driver, echo :'%d'\n", *buf);
...
static int nav_input_thread(void *et_spi)
{
	...
	egislog_d("nav_input_thread enter\n");
	...
	egislog_d("nav_input_thread exit\n");
...
void uinput_egis_init(struct egistec_data *egistec)
{
	...
	egislog_d("Egis navigation driver, uinput init\n");
	...
	if (!egistec->input_dev) {
		egislog_e("Egis navigation driver, Input_allocate_device failed.\n");
		return;
	}
	...
	if (error) {
		egislog_e("Egis navigation driver, Input_register_device failed.\n");
		input_free_device(egistec->input_dev);
		egistec->input_dev = NULL;
	}
	g_DoubleClickJiffies = 0;
	g_SingleClickJiffies = 0;
	egislog_d("Egis navigation driver, uinput init OK\n");
}

void uinput_egis_destroy(struct egistec_data *egistec)
{
	egislog_d("Egis navigation driver uinput destroy\n");
...
void sysfs_egis_init(struct egistec_data *egistec)
{
	...
	egislog_d("Egis navigation driver, egis_input device init\n");
	egistec->pd = platform_device_alloc("egis_input", -1);
	if (!egistec->pd) {
		egislog_e("Egis navigation driver, platform_device_alloc fail\n");
		return;
	}

	status = platform_device_add(egistec->pd);
	if (status != 0) {
		egislog_e("Egis navigation driver, platform_device_add fail\n");
	...
	if (status) {
		egislog_e("Egis navigation driver, could not create sysfs\n");
		platform_device_del(egistec->pd);
		platform_device_put(egistec->pd);
		return;
	}
	egislog_d("Egis navigation driver, egis_input device OK\n");
}

void sysfs_egis_destroy(struct egistec_data *egistec)
{
	egislog_d("Egis navigation driver sysfs destroy\n");
