...
static int sf_ctl_device_power_by_regulator(bool on)
{
    ...
    if (on && !isPowerOn) {
        ...
        if (err) {
            sf_debug(ERR_LOG, "Regulator vdd enable failed, err=%d\n", err);
            return err;
        }
...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
    ...
    if (IS_ERR(ctl_dev->vdd_reg)) {
        ...
        sf_debug(ERR_LOG, "Regulator get failed, err=%d\n", err);
        return err;
    }
    ...
    if (regulator_count_voltages(ctl_dev->vdd_reg) > 0) {
        ...
        if (err) {
            sf_debug(ERR_LOG, "Regulator set_vtg failed, err=%d\n", err);
            return err;
        }
    }
#endif
