...
/*zte add led current control node start*/
#define SC2721_RGB_V_DEFAULT	BIT(6) /*add default current 7.5 mA*/
#define SC2721_RGB_V_MASK  GENMASK(8, 4)  /*add current mask*/
#define SC2721_RGB_V_SHIFT	4
int zte_rgb_v_current = 0;
/*zte add led current control node end*/
...
/*zte add led current control node start*/
	...
	regmap_update_bits(regmap, data->rgb_ctrl, SC2721_RGB_V_MASK, zte_rgb_v_current);
#endif
/*zte add led current control node end*/
...
/*zte add led current control node start*/
	err = of_property_read_u32(np, "rgb_v_current", &zte_rgb_v_current);
	if (err) {
		dev_info(dev, "rgb_v_current is default.\n");
		zte_rgb_v_current = SC2721_RGB_V_DEFAULT;
	} else {
		zte_rgb_v_current = zte_rgb_v_current << SC2721_RGB_V_SHIFT;
	}
/*zte add led current control node end*/
